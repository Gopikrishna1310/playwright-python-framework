...
def test_Create_Invalid_Project_Validation_Errors(before_each):
    ...
    try:
        ...
        db_query_names = test_data_inputs.db_query_project_names
        ...
        duplicate_error_visible = action_factory.ui_utils.is_element_visible(action_factory.page_factory.projects_page.duplicate_project_error, timeout=5000) or \
                                 action_factory.ui_utils.is_element_visible(action_factory.page_factory.projects_page.duplicate_project_warning, timeout=5000)
        
        if duplicate_error_visible:
            action_factory.helpers.attach_screenshot(name="DuplicateProjectErrorVisible")
            action_factory.helpers.attach_allure(name="Duplicate Project Validation", text=f"Error displayed when using duplicate project name: {existing_project_name}")
        else:
            status = "Fail"
        ...
        action_factory.projects_actions.close_create_project_modal()

        # Special Characters Validation
        action_factory.projects_actions.create_project(project_name=special_char_name)
        ...
        if special_char_error_visible:
            ...
            action_factory.helpers.attach_allure(name="Special Characters Validation", text=f"Error displayed for special character name: {special_char_name}")
        else:
            status = "Fail"
        ...
        for db_query_name in db_query_names:
            action_factory.projects_actions.create_project(project_name=db_query_name)
            ...
            if db_query_error_visible:
                action_factory.helpers.attach_screenshot(name=f"DBQueryErrorVisible_{db_query_name}")
                action_factory.helpers.attach_allure(name="DB Query Name Validation", text=f"Error displayed for DB query name: {db_query_name}")
            else:
                status = "Fail"
                message = f"Invalid character/query error message not displayed for DB query name '{db_query_name}'."
                action_factory.helpers.attach_screenshot(name=f"DBQueryErrorMissing_{db_query_name}")
        ...
        status = "Pass"
        message = "All invalid project creation scenarios validated successfully."
        action_factory.helpers.attach_screenshot(name="InvalidProjectValidationsPass")
        action_factory.helpers.attach_allure(name="Invalid Project Validation Success", text=message)
        assert True, message
