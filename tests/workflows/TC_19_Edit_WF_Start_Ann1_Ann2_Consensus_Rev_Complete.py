...
@allure.story("TC_19: Start -> Annotate 1 -> Annotate 2 Consensus -> Review -> Complete (Edit)")
@allure.title("Verify creating, editing, and saving a TC_19 workflow with Annotate 2 in Consensus Mode (2 Annotators)")
def test_tc19_edit_wf_start_ann1_ann2_consensus_rev_complete(before_each):
    status = "Fail"
    message = ""
    try:
        ...
        # Set Required Annotators = 2 for Annotate 2 (Consensus Mode)
        ...
        if workflow_name in workflow_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC19WorkflowEditPass")
            action_factory.helpers.attach_allure(name="TC_19 Edit Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' not found after edit."
            action_factory.helpers.attach_screenshot(name="TC19WorkflowEditFailed")
            action_factory.helpers.attach_allure(name="TC_19 Edit Workflow", text=message)
            assert False, message
