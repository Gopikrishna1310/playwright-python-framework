...
@allure.story("TC_08: Start -> Annotate -> Review 1 -> Review 2 -> Complete (Delete Linked)")
@allure.title("Verify attempting to delete a TC_08 workflow linked to an active project is blocked")
def test_tc08_delete_wf_start_ann_rev1_rev2_complete(before_each):
    status = "Fail"
    message = ""
    try:
        ...
        workflow_name = test_data_inputs.tc08_delete_workflow_name
        project_name = test_data_inputs.tc08_project_name
        ...
        if error_visible and workflow_name in remaining_workflows:
            ...
            action_factory.helpers.attach_screenshot(name="TC08PreventDeleteLinkedWorkflowPass")
