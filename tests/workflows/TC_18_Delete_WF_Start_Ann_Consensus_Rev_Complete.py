...
@allure.story("TC_18: Start -> Annotate Consensus -> Review -> Complete (Delete Linked)")
@allure.title("Verify attempting to delete a TC_18 workflow linked to an active project is blocked")
def test_tc18_delete_wf_start_ann_consensus_rev_complete(before_each):
    status = "Fail"
    message = ""
    try:
        ...
        workflow_name = test_data_inputs.tc18_delete_workflow_name
        project_name = test_data_inputs.tc18_project_name
        ...
        if error_visible and workflow_name in remaining_workflows:
            ...
            action_factory.helpers.attach_screenshot(name="TC18PreventDeleteLinkedWorkflowPass")
            action_factory.helpers.attach_allure(name="TC_09 Delete Linked Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Linked workflow '{workflow_name}' was unexpectedly deleted."
            action_factory.helpers.attach_screenshot(name="TC09PreventDeleteLinkedWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_09 Delete Linked Workflow", text=message)
