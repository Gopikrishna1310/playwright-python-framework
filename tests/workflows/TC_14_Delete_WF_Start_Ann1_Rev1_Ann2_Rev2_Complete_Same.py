...
@allure.story("TC_14: Start -> Annotate 1 -> Review 1 -> Annotate 2 -> Review 2 -> Complete Same Template (Delete Linked)")
@allure.title("Verify attempting to delete a TC_14 workflow linked to an active project is blocked")
def test_tc14_delete_wf_start_ann1_rev1_ann2_rev2_complete_same(before_each):
    status = "Fail"
    message = ""
    try:
        ...
        workflow_name = test_data_inputs.tc14_delete_workflow_name
        project_name = test_data_inputs.tc14_project_name
        ...
        if error_visible and workflow_name in remaining_workflows:
            ...
            action_factory.helpers.attach_screenshot(name="TC14PreventDeleteLinkedWorkflowPass")
            action_factory.helpers.attach_allure(name="TC_07 Delete Linked Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Linked workflow '{workflow_name}' was unexpectedly deleted."
            action_factory.helpers.attach_screenshot(name="TC07PreventDeleteLinkedWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_07 Delete Linked Workflow", text=message)
