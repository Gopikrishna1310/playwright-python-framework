...
@allure.story("TC_11: Start -> Annotate 1 -> Annotate 2 -> Complete Diff Templates (Edit)")
@allure.title("Verify creating, editing, and saving a TC_11 workflow with different templates on two Annotate nodes connected directly to Complete")
def test_tc11_edit_wf_start_ann1_ann2_complete_diff(before_each):
    status = "Fail"
    message = ""
    try:
        ...
        workflow_name = test_data_inputs.tc11_edit_workflow_name
        ...
        if workflow_name in workflow_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC11WorkflowEditPass")
            action_factory.helpers.attach_allure(name="TC_11 Edit Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' not found after edit."
            action_factory.helpers.attach_screenshot(name="TC11WorkflowEditFailed")
            action_factory.helpers.attach_allure(name="TC_11 Edit Workflow", text=message)
            assert False, message
