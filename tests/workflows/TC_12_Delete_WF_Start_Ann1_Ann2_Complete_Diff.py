...
@allure.story("TC_12: Start -> Annotate 1 -> Annotate 2 -> Complete Diff Templates (Delete Linked)")
@allure.title("Verify attempting to delete a TC_12 workflow linked to an active project is blocked")
def test_tc12_delete_wf_start_ann1_ann2_complete_diff(before_each):
    status = "Fail"
    message = ""
    try:
        ...
        workflow_name = test_data_inputs.tc12_delete_workflow_name
        project_name = test_data_inputs.tc12_project_name
        ...
        nodes_list = ["Start", "Annotate", "Annotate", "Complete"]
        ...
        if error_visible and workflow_name in remaining_workflows:
            ...
            action_factory.helpers.attach_screenshot(name="TC12PreventDeleteLinkedWorkflowPass")
            action_factory.helpers.attach_allure(name="TC_06 Delete Linked Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Linked workflow '{workflow_name}' was unexpectedly deleted."
            action_factory.helpers.attach_screenshot(name="TC06PreventDeleteLinkedWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_06 Delete Linked Workflow", text=message)
