...
@allure.story("TC_09: Start -> Annotate 1 -> Annotate 2 -> Complete Same Template (Edit)")
@allure.title("Verify creating, editing, and saving a TC_09 workflow with same template on two Annotate nodes connected directly to Complete")
def test_tc09_edit_wf_start_ann1_ann2_complete_same(before_each):
    status = "Fail"
    message = ""
    try:
        action_factory = before_each
        template_name = test_data_inputs.common_template_name_A
        workflow_name = test_data_inputs.tc09_edit_workflow_name
        ...
        if workflow_name in workflow_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC09WorkflowEditPass")
            action_factory.helpers.attach_allure(name="TC_09 Edit Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' not found after edit."
            action_factory.helpers.attach_screenshot(name="TC09WorkflowEditFailed")
            action_factory.helpers.attach_allure(name="TC_09 Edit Workflow", text=message)
            assert False, message
