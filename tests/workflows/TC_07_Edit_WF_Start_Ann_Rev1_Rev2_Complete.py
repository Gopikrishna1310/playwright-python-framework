...
@allure.story("TC_07: Start -> Annotate -> Review 1 -> Review 2 -> Complete (Edit)")
@allure.title("Verify creating, editing, and saving a TC_07 workflow with two Review nodes")
def test_tc07_edit_wf_start_ann_rev1_rev2_complete(before_each):
    status = "Fail"
    message = ""
    try:
        action_factory = before_each
        template_name = test_data_inputs.common_template_name_A
        workflow_name = test_data_inputs.tc07_edit_workflow_name
        description = test_data_inputs.description
        template_file = test_data_inputs.valid_template_file
        nodes_list = ["Start", "Annotate", "Review", "Review", "Complete"]
        ...
        if workflow_name in workflow_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC07WorkflowEditPass")
            action_factory.helpers.attach_allure(name="TC_07 Edit Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' not found after edit."
            action_factory.helpers.attach_screenshot(name="TC07WorkflowEditFailed")
            action_factory.helpers.attach_allure(name="TC_07 Edit Workflow", text=message)
            assert False, message
