...
@allure.story("TC_10: Start -> Annotate 1 -> Annotate 2 -> Complete Same Template (Delete Linked)")
@allure.title("Verify attempting to delete a TC_10 workflow linked to an active project is blocked")
def test_tc10_delete_wf_start_ann1_ann2_complete_same(before_each):
    status = "Fail"
    message = ""
    try:
        ...
        workflow_name = test_data_inputs.tc10_delete_workflow_name
        project_name = test_data_inputs.tc10_project_name
        ...
        if error_visible and workflow_name in remaining_workflows:
            ...
            action_factory.helpers.attach_screenshot(name="TC10PreventDeleteLinkedWorkflowPass")
            action_factory.helpers.attach_allure(name="TC_05 Delete Linked Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Linked workflow '{workflow_name}' was unexpectedly deleted."
            action_factory.helpers.attach_screenshot(name="TC05PreventDeleteLinkedWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_05 Delete Linked Workflow", text=message)
