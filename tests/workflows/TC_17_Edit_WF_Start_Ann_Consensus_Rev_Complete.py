...
@allure.story("TC_17: Start -> Annotate Consensus -> Review -> Complete (Edit)")
@allure.title("Verify creating, editing, and saving a TC_17 workflow with Consensus Mode (2 Annotators)")
def test_tc17_edit_wf_start_ann_consensus_rev_complete(before_each):
    status = "Fail"
    message = ""
    try:
        action_factory = before_each
        template_name = test_data_inputs.common_template_name_A
        workflow_name = test_data_inputs.tc17_edit_workflow_name
        ...
        # Set Required Annotators = 2 (Consensus Mode)
        ...
        if workflow_name in workflow_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC17WorkflowEditPass")
            action_factory.helpers.attach_allure(name="TC_17 Edit Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' not found after edit."
            action_factory.helpers.attach_screenshot(name="TC17WorkflowEditFailed")
            action_factory.helpers.attach_allure(name="TC_17 Edit Workflow", text=message)
            assert False, message
