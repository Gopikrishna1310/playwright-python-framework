...
@allure.story("TC_16: Start -> Annotate 1 -> Review 1 -> Annotate 2 -> Review 2 -> Complete Diff Templates (Delete Linked)")
@allure.title("Verify attempting to delete a TC_16 workflow linked to an active project is blocked")
def test_tc16_delete_wf_start_ann1_rev1_ann2_rev2_complete_diff(before_each):
    status = "Fail"
    message = ""
    try:
        ...
        workflow_name = test_data_inputs.tc16_delete_workflow_name
        project_name = test_data_inputs.tc16_project_name
        ...
        if error_visible and workflow_name in remaining_workflows:
            ...
            action_factory.helpers.attach_screenshot(name="TC16PreventDeleteLinkedWorkflowPass")
            action_factory.helpers.attach_allure(name="TC_08 Delete Linked Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Linked workflow '{workflow_name}' was unexpectedly deleted."
            action_factory.helpers.attach_screenshot(name="TC08PreventDeleteLinkedWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_08 Delete Linked Workflow", text=message)
