...
@allure.story("TC_05: Start -> Annotate 1 -> Annotate 2 -> Review -> Complete Diff Templates (Edit)")
@allure.title("Verify creating, editing, and saving a TC_05 workflow with different templates on two Annotate nodes")
def test_tc05_edit_wf_start_ann1_ann2_rev_complete_diff(before_each):
    status = "Fail"
    message = ""
    try:
        ...
        workflow_name = test_data_inputs.tc05_edit_workflow_name
        ...
        if workflow_name in workflow_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC05WorkflowEditPass")
            action_factory.helpers.attach_allure(name="TC_05 Edit Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' not found after edit."
            action_factory.helpers.attach_screenshot(name="TC05WorkflowEditFailed")
            action_factory.helpers.attach_allure(name="TC_05 Edit Workflow", text=message)
            assert False, message
