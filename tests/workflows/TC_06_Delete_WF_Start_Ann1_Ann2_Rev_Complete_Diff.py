...
@allure.story("TC_06: Start -> Annotate 1 -> Annotate 2 -> Review -> Complete Diff Templates (Delete Linked)")
@allure.title("Verify attempting to delete a TC_06 workflow linked to an active project is blocked")
def test_tc06_delete_wf_start_ann1_ann2_rev_complete_diff(before_each):
    status = "Fail"
    message = ""
    try:
        ...
        workflow_name = test_data_inputs.tc06_delete_workflow_name
        project_name = test_data_inputs.tc06_project_name
        ...
        if error_visible and workflow_name in remaining_workflows:
            ...
            action_factory.helpers.attach_screenshot(name="TC06PreventDeleteLinkedWorkflowPass")
            action_factory.helpers.attach_allure(name="TC_03 Delete Linked Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Linked workflow '{workflow_name}' was unexpectedly deleted."
            action_factory.helpers.attach_screenshot(name="TC03PreventDeleteLinkedWorkflowFailed")
            action_factory.helpers.attach_allure(name="TC_03 Delete Linked Workflow", text=message)
