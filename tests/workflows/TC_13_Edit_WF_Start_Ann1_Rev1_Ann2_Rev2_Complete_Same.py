...
@allure.story("TC_13: Start -> Annotate 1 -> Review 1 -> Annotate 2 -> Review 2 -> Complete Same Template (Edit)")
@allure.title("Verify creating, editing, and saving a TC_13 workflow with same template on alternating Annotate/Review nodes")
def test_tc13_edit_wf_start_ann1_rev1_ann2_rev2_complete_same(before_each):
    status = "Fail"
    message = ""
    try:
        action_factory = before_each
        template_name = test_data_inputs.common_template_name_A
        workflow_name = test_data_inputs.tc13_edit_workflow_name
        ...
        if workflow_name in workflow_list:
            ...
            action_factory.helpers.attach_screenshot(name="TC13WorkflowEditPass")
            action_factory.helpers.attach_allure(name="TC_13 Edit Workflow", text=message)
            assert True, message
        else:
            status = "Fail"
            message = f"Workflow '{workflow_name}' not found after edit."
            action_factory.helpers.attach_screenshot(name="TC13WorkflowEditFailed")
            action_factory.helpers.attach_allure(name="TC_13 Edit Workflow", text=message)
            assert False, message
