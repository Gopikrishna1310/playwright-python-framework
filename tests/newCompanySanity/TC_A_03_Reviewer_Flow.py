...
def test_Reviewer_Flow_test(before_each):
    ...
    try: 
        ...
        Annotation1 = test_data_inputs.TC_A_annotation_text_1
        Annotation3 = test_data_inputs.TC_A_annotation_text_3
        Annotation4 = test_data_inputs.TC_A_annotation_text_4
        Annotation_Edit = test_data_inputs.TC_A_annotation_text_Edit
        Reviewer_Edit = test_data_inputs.TC_A_reviewer_text_Edit
        Reviewer_Approve = test_data_inputs.TC_A_reviewer_text_Approve
        ...
        if all(file in files_name_list for file in Dataset_files_upload):
            ...
            action_factory.helpers.attach_allure(name="Uploaded Files", text=", ".join(Dataset_files_upload))
            assert False, message
        # perform Review Functionality for 1st file 
        ...
        # Validate Annotator Transcription
        action_factory.ui_utils.element_wait_for(action_factory.page_factory.annotator_page.transcription.nth(0),timeout=50000)
        annotator_transcription = action_factory.ui_utils.grab_text_from_all(action_factory.page_factory.annotator_page.transcription)
        if Annotation1 in annotator_transcription and Annotation_Edit in annotator_transcription and Annotation3 not in annotator_transcription and Annotation4 in annotator_transcription:
            status = "Pass"
            message = f"Transcription text matches the annotation text."
            action_factory.helpers.attach_screenshot(name="Annotataor Transcription Matched")
            action_factory.helpers.attach_allure(name="Annotator Transcription", text="\n".join(annotator_transcription))
            assert True, message
        else:
            status = "Fail"
            message = f"Transcription text does not match the annotation text."
            action_factory.helpers.attach_screenshot(name="Annotataor Transcription Not Matched")
            action_factory.helpers.attach_allure(name="Annotator Transcription", text="\n".join(annotator_transcription))
            assert False, message
        # Validate Reviewer Transcription before save and exit
        action_factory.annotator_actions.annontate_files(Reviewer_Approve, fast_forward = "15")
        transcription = action_factory.ui_utils.grab_text_from_all(action_factory.page_factory.annotator_page.transcription)
        if Reviewer_Approve in transcription:
            status = "Pass"
            message = f"Transcription text matches the reviewer text."
            action_factory.helpers.attach_screenshot(name="Reviewer Transcription Matched")
            action_factory.helpers.attach_allure(name="Reviewer Transcription", text="\n".join(transcription))
            assert True, message
        else:
            status = "Fail"
            message = f"Reviewer text does not match the reviewer text."
            action_factory.helpers.attach_screenshot(name="Reviewer Transcription Not Matched")
            action_factory.helpers.attach_allure(name="Reviewer Transcription", text="\n".join(transcription))
            assert False, message
        ...
        if Reviewer_Approve in reviewer_transcription and Annotation1 in reviewer_transcription and Annotation_Edit in reviewer_transcription and Annotation4 in reviewer_transcription:
            status = "Pass"
        ...
        action_factory.annotator_actions.perform_edit_transcription(Annotation4, Reviewer_Edit)
        # Validate Reviewer Transcription after edit
        reviewer_transcription = action_factory.ui_utils.grab_text_from_all(action_factory.page_factory.annotator_page.transcription)
        if Reviewer_Approve in reviewer_transcription and Reviewer_Edit in reviewer_transcription and Annotation1 in reviewer_transcription and Annotation_Edit in reviewer_transcription and Annotation4 not in reviewer_transcription:
            status = "Pass"
            message = f"Reviewer Transcription matches the reviewer text."
            action_factory.helpers.attach_screenshot(name="ReviewerTranscriptionMatches")
            action_factory.helpers.attach_allure(name="Reviewer Transcription", text="\n".join(reviewer_transcription))
        ...
        transcription = action_factory.ui_utils.grab_text_from_all(action_factory.page_factory.annotator_page.transcription)
        if Annotation1 in transcription and Annotation_Edit in transcription and Annotation4 not in transcription and Reviewer_Approve in transcription and Reviewer_Edit in transcription:
            status = "Pass"
        ...
        if count > 0:
            ...
            if Dataset_files_upload[0] not in files_list:
                status = "Pass"
                message = f"Reviewer completed the 1st file"
                action_factory.helpers.attach_screenshot(name="ReviewerCompleted1stFile")
                action_factory.helpers.attach_allure(name="Reviewer Completed 1st File", text="Reviewer")
                assert True, message
            else:
                status = "Fail"
                message = f"Annotator does not completed the 1st file"
                action_factory.helpers.attach_screenshot(name="AnnotatorNotCompleted1stFile")
                action_factory.helpers.attach_allure(name="Annotator Not Completed 1st File", text="Annotator")
                assert False, message
        else:
            status = "Pass"
        ...
        # perform Review Functionality for 2nd file 
        ...
        if is_visible:
            status = "Pass"
            message = f"Begin Recording button is visible for 2nd file"
            action_factory.helpers.attach_screenshot(name="Begin Recording button 2nd file")
            action_factory.helpers.attach_allure(name="Begin Recording button 2nd file", text="Begin Recording button is visible for 2nd file")
            assert True, message
        else:
            status = "Fail"
            message = f"Begin Recording button is not visible for 2nd file."
            action_factory.helpers.attach_screenshot(name="Begin Recording button 2nd file Not Visible")
            action_factory.helpers.attach_allure(name="Begin Recording button 2nd file Not Visible", text="Begin Recording button is not visible for 2nd file")
            assert False, message
        action_factory.ui_utils.smart_wait()
        # Validate Annotator Transcription
        action_factory.ui_utils.element_wait_for(action_factory.page_factory.annotator_page.transcription.nth(0),timeout=50000)
        annotator_transcription = action_factory.ui_utils.grab_text_from_all(action_factory.page_factory.annotator_page.transcription)
        if Annotation1 in annotator_transcription and Annotation_Edit in annotator_transcription and Annotation4 in annotator_transcription and Annotation3 not in annotator_transcription:
            status = "Pass"
            message = f"Transcription text matches the annotation text."
            action_factory.helpers.attach_screenshot(name="Annotataor Transcription Matched")
            action_factory.helpers.attach_allure(name="Annotator Transcription", text="\n".join(annotator_transcription))
        ...
        # Validate Reviewer Transcription after save and exit
        reviewer_transcription = action_factory.ui_utils.grab_text_from_all(action_factory.page_factory.annotator_page.transcription)
        if Reviewer_Approve in reviewer_transcription and Annotation1 in reviewer_transcription and Annotation_Edit in reviewer_transcription and Annotation4 in reviewer_transcription and Annotation3 not in annotator_transcription:
            status = "Pass"
            message = f"Reviewer Transcription matches the reviewer text."
            action_factory.helpers.attach_screenshot(name="ReviewerTranscriptionMatches")
            action_factory.helpers.attach_allure(name="Reviewer Transcription", text="\n".join(reviewer_transcription))
