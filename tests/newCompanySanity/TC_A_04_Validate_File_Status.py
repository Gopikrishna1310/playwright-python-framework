...
def test_Validate_File_status_test(before_each):
    ...
    try: 
        ...
        i = action_factory.helpers.resolve_latest_index(project_name_list, test_data_inputs.TC_A_project_name)
        ...
        for file_Name in Dataset_files_upload:
            action_factory.ui_utils.smart_wait()
            status = action_factory.ui_utils.grab_text_from_all(action_factory.page_factory.projects_page.get_file_status(file_Name))
            if "Complete" in status:
                status = "Pass"
                message = f"File status '{Dataset_files_upload}' completed successfully."
                action_factory.helpers.attach_screenshot(name=f"{file_Name}File Status Completed")
                action_factory.helpers.attach_allure(name=f"{file_Name}File Status", text=file_Name)
                assert True, message
            else:
                status = "Fail"
                message = f"File status '{Dataset_files_upload}' completion failed."
                action_factory.helpers.attach_screenshot(name=f"{file_Name}File Status Completion Failed")
                action_factory.helpers.attach_allure(name=f"{file_Name}File Status", text=file_Name)
                assert False, message

        # Export the Json File and Validate that the Annotator and Reviewer transcription are shown properly
        Annotation1 = test_data_inputs.TC_A_annotation_text_1
        Annotation2 = test_data_inputs.TC_A_annotation_text_2
        ...
        expected_texts = [Annotation1, Annotation_Edit, Reviewer_Edit, Reviewer_Approve]
        unexpected_texts = [Annotation2, Annotation3, Annotation4]
        ...
        if file_transcriptions:
            status = "Pass"
            message = f"Exported JSON file contains expected texts {expected_texts} and does not contain deleted/edited texts {unexpected_texts} for files {Dataset_files_upload}."
            ...
            status = "Fail"
            message = "Exported JSON file validation failed."
