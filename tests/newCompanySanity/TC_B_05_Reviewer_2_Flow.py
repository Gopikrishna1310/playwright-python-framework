...
def before_each(page):
    ...
    email = test_data_inputs.get_reviewer_2_email(action_factory.helpers)
...
@allure.feature("Reviewer 2")
@allure.story("Reviewer 2 Flow")
@allure.title("Reviewer 2 Flow Validation")
def test_Reviewer_Flow_test(before_each):
    ...
    try: 
        ...
        action_factory.annotator_actions.annontate_files(TC_B_reviewer_2_text_Approve, fast_forward = "50")
        # Validate Reviewer Annotation
        transcription = action_factory.ui_utils.grab_text_from_all(action_factory.page_factory.annotator_page.transcription)
        if TC_B_reviewer_2_text_Approve in transcription:
            status = "Pass"
            message = f"Transcription text matches the input text."
            action_factory.helpers.attach_screenshot(name="TranscriptionMatches")
            action_factory.helpers.attach_allure(name="Transcription", text="\n".join(transcription))
        ...
        if TC_B_reviewer_2_text_Approve in reviewer_transcription:
            status = "Pass"
        ...
        action_factory.annotator_actions.perform_edit_transcription(TC_B_annotation_2_2, TC_B_reviewer_2_text_Edit)
        # Validate Reviewer Transcription after edit
        reviewer_transcription = action_factory.ui_utils.grab_text_from_all(action_factory.page_factory.annotator_page.transcription)
        if TC_B_reviewer_2_text_Edit in reviewer_transcription and TC_B_annotation_2_2 not in reviewer_transcription:
            status = "Pass"
        ...
        Transcription_to_have = TC_B_annotation_1_1 in transcription and TC_B_annotation_1_Edit in transcription and TC_B_reviewer_1_text_Edit in transcription and TC_B_reviewer_1_text_Approve in transcription and TC_B_annotation_2_1 in transcription and TC_B_annotation_2_Edit in transcription
        Transcription_do_not_have = TC_B_annotation_1_2 not in transcription and TC_B_annotation_1_3 not in transcription and TC_B_annotation_1_4 not in transcription and TC_B_annotation_2_2 not in transcription and TC_B_annotation_2_3 not in transcription and TC_B_annotation_2_4 not in transcription
        if Transcription_to_have and Transcription_do_not_have:
            status = "Pass"
        ...
        # Review 2nd file Claim , Review and Annotate
        action_factory.ui_utils.smart_wait()
        ...
        action_factory.ui_utils.click_element(action_factory.page_factory.reviewer_page.reject_button)
        action_factory.ui_utils.click_element(action_factory.page_factory.reviewer_page.submit_button)
        action_factory.ui_utils.smart_wait()
        claim_visible = action_factory.ui_utils.wait_for_visible_if_exists(action_factory.page_factory.annotator_page.claim_button, timeout=10000)
        if claim_visible:
            action_factory.ui_utils.click_element(action_factory.page_factory.annotator_page.back_button)
