...
@allure.feature("Company Admin")
@allure.story("Company Admin Flow")
@allure.title("Company Admin Flow Validation - TC_B_01")
def test_company_admin_B_flow_test(before_each):
    status = "Fail"
    message = ""
    try: 
        action_factory = before_each
        
        dataset_name = test_data_inputs.TC_B_dataset_name
        template_name = test_data_inputs.TC_B_template_name
        template_name_2 = test_data_inputs.TC_B_template_name_2
        workflow_name = test_data_inputs.TC_B_workflow_name
        project_name = test_data_inputs.TC_B_project_name
        dataset_type_name = test_data_inputs.TC_A_dataset_type_name
        Dataset_files_upload = test_data_inputs.TC_A_Dataset_files_upload
        TC_B_Template_files_upload_1 = test_data_inputs.TC_B_Template_files_upload_1
        TC_B_Template_files_upload_2 = test_data_inputs.TC_B_Template_files_upload_2
        nodes_list = test_data_inputs.TC_B_nodes_list
        ...
        i = action_factory.helpers.resolve_next_index(dataset_names_list, test_data_inputs.TC_B_dataset_name)

        dataset_name = test_data_inputs.TC_B_dataset_name.format(i=i) if "{i}" in test_data_inputs.TC_B_dataset_name else test_data_inputs.TC_B_dataset_name
        template_name = test_data_inputs.TC_B_template_name.format(i=i) if "{i}" in test_data_inputs.TC_B_template_name else test_data_inputs.TC_B_template_name
        template_name_2 = test_data_inputs.TC_B_template_name_2.format(i=i) if "{i}" in test_data_inputs.TC_B_template_name_2 else test_data_inputs.TC_B_template_name_2
        workflow_name = test_data_inputs.TC_B_workflow_name.format(i=i) if "{i}" in test_data_inputs.TC_B_workflow_name else test_data_inputs.TC_B_workflow_name
        ...
        action_factory.templates_actions.upload_template_files(*TC_B_Template_files_upload_1)
        action_factory.ui_utils.smart_wait()
        # 2nd Template
        action_factory.templates_actions.click_templates_menu()
        action_factory.templates_actions.upload_new_template(template_name=template_name_2)
        action_factory.templates_actions.upload_template_files(*TC_B_Template_files_upload_2)
        action_factory.ui_utils.smart_wait()
        ...
        if template_name in template_name_list and template_name_2 in template_name_list:
            status = "Pass"
            message = f"Template '{template_name}' and '{template_name_2}' created successfully."
            action_factory.helpers.attach_screenshot(name="TemplateCreated")
            action_factory.helpers.attach_allure(name="Template Name", text=template_name + ", " + template_name_2)
            assert True, message
        else:
            status = "Fail"
            message = f"Template '{template_name}' or '{template_name_2}' creation failed."
        ...
        action_factory.ui_utils.smart_wait()
        action_factory.workflows_actions.apply_template_to_annotate(template_name=template_name_2, position=0)
        action_factory.ui_utils.smart_wait()
        ...
        if template_name_2 in template_applied and template_name in template_applied:
            status = "Pass"
            message = f"Workflow '{workflow_name}' created successfully with template '{template_name}' and '{template_name_2}' applied."
            ...
            status = "Fail"
            message = f"Workflow '{workflow_name}' creation failed or template '{template_name}' and '{template_name_2}' not applied."
